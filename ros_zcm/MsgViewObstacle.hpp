...
#ifndef __MsgViewObstacle_hpp__
#define __MsgViewObstacle_hpp__

#include <vector>
#include "MsgViewObstacleTrajectoryPoint.hpp"


class MsgViewObstacle
{
    public:
        double     x;
        ...
        std::vector< MsgViewObstacleTrajectoryPoint > poses;

    public:
        /**
         * Destructs a message properly if anything inherits from it
        */
        virtual ~MsgViewObstacle() {}
...
int MsgViewObstacle::encode(void* buf, uint32_t offset, uint32_t maxlen) const
{
    uint32_t pos = 0;
    int thislen;
...
int MsgViewObstacle::decode(const void* buf, uint32_t offset, uint32_t maxlen)
{
    uint32_t pos = 0;
    int thislen;
...
uint32_t MsgViewObstacle::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t MsgViewObstacle::getHash()
{
    static int64_t hash = _computeHash(NULL);
    return hash;
}

const char* MsgViewObstacle::getTypeName()
{
    return "MsgViewObstacle";
}

int MsgViewObstacle::_encodeNoHash(void* buf, uint32_t offset, uint32_t maxlen) const
{
    uint32_t pos = 0;
    int thislen;
...
int MsgViewObstacle::_decodeNoHash(const void* buf, uint32_t offset, uint32_t maxlen)
{
    uint32_t pos = 0;
    int thislen;
...
uint32_t MsgViewObstacle::_getEncodedSizeNoHash() const
{
    uint32_t enc_size = 0;
...
uint64_t MsgViewObstacle::_computeHash(const __zcm_hash_ptr* p)
{
    const __zcm_hash_ptr* fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == MsgViewObstacle::getHash)
            return 0;
    const __zcm_hash_ptr cp = { p, (void*)MsgViewObstacle::getHash };

    uint64_t hash = (uint64_t)0xda565f6d2a34a4a9LL +
         MsgViewObstacleTrajectoryPoint::_computeHash(&cp);
